...
app.patch("/:volunteerUserName", AuthChecker, (req, res) => {
	...
	if (foundVolunteer) {
		...
		if (req.body.emergencyContactHomePhone) {
			foundVolunteer.emergencyContactHomePhone = parseInt(
				req.body.emergencyContactHomePhone
			);
		}
		if (req.body.emergencyContactWorkPhone) {
			foundVolunteer.emergencyContactWorkPhone = parseInt(
				req.body.emergencyContactWorkPhone
			);
		}
...
app.delete("/:volunteerUserName", AuthChecker, (req, res) => {
	let newList: Volunteer[] = [];
	for (let volunteer of volunteerList) {
		if (volunteer.userName === req.params.volunteerUserName) {
			newList = volunteerList.filter(
				(volunteer) =>
					volunteer.userName !== req.params.volunteerUserName
			);
